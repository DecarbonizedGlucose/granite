...
func (p *sessionRecord) encode(w io.Writer) error {
	...
	if p.has(recComparer) {
		p.putVarint(w, recComparer)
		p.putBytes(w, []byte(p.comparer))
	}
	if p.has(recJournalNum) {
		p.putVarint(w, recJournalNum)
		p.putVarint(w, p.journalNum)
	}
	if p.has(recNextFileNum) {
		p.putVarint(w, recNextFileNum)
		p.putVarint(w, p.nextFileNum)
	}
	if p.has(recSeqNum) {
		p.putVarint(w, recSeqNum)
		p.putUvarint(w, p.seqNum)
	}
	for _, cp := range p.compPtrs {
		p.putUvarint(w, recCompPtr)
		p.putUvarint(w, uint64(cp.level))
		p.putBytes(w, cp.ikey)
	}
	for _, dt := range p.deletedTables {
		p.putUvarint(w, recDelTable)
		p.putUvarint(w, uint64(dt.level))
		p.putVarint(w, dt.num)
	}
	for _, at := range p.addedTables {
		p.putUvarint(w, recAddTable)
		p.putUvarint(w, uint64(at.level))
		p.putVarint(w, at.num)
		p.putVarint(w, at.size)
		p.putBytes(w, at.imin)
		p.putBytes(w, at.imax)
	}
	return p.err
}
...
func (p *sessionRecord) readUvarintMayEOF(field string, r io.ByteReader, mayEOF bool) uint64 {
	...
	if err != nil {
		if err == io.ErrUnexpectedEOF || (!mayEOF && err == io.EOF) {
			p.err = gerrors.NewErrFileCorrupted(
				util.FileDesc{},
				&gerrors.ErrManifestCorrupted{Field: field, Reason: "short read"},
			)
		} else if strings.HasPrefix(err.Error(), "binary:") {
			p.err = gerrors.NewErrFileCorrupted(
				util.FileDesc{},
				&gerrors.ErrManifestCorrupted{Field: field, Reason: err.Error()},
			)
		} else {
			p.err = err
		}
...
func (p *sessionRecord) readVarint(field string, r io.ByteReader) int64 {
	...
	if x < 0 {
		p.err = gerrors.NewErrFileCorrupted(
			util.FileDesc{},
			&gerrors.ErrManifestCorrupted{Field: field, Reason: "invalid negative value"},
		)
	}
	return x
}
...
func (p *sessionRecord) decode(r io.Reader) error {
	...
	for p.err == nil {
		...
		case recCompPtr:
			lvl := p.readLevel("comp-ptr.level", br)
			ikey := p.readBytes("comp-ptr.ikey", br)
			if p.err == nil {
				p.addCompPtr(lvl, internalKey(ikey))
			}
		case recAddTable:
			lvl := p.readLevel("add-table.level", br)
			num := p.readVarint("add-table.num", br)
			size := p.readVarint("add-table.size", br)
			imin := p.readBytes("add-table.imin", br)
			imax := p.readBytes("add-table.imax", br)
			if p.err == nil {
				p.addTable(lvl, num, size, internalKey(imin), internalKey(imax))
			}
		case recDelTable:
			lvl := p.readLevel("del-table.level", br)

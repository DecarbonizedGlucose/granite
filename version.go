...
type version struct {
	...
	// Score < 1 means compaction is not strictly needed. These
	// fields will be initialized by computeCompaction()
	cLevel int
	cScore float64
...
func (v *version) incRef() {
	...
	if v.ref == 1 {
		...
		case v.s.refCh <- &vTask{vid: v.id, files: v.levels, created: time.Now()}:
			// v.levels is immutable, we can use it directly
		case <-v.s.closeC:
...
// Scan table files that overlap with the target key range layer by layer.
func (v *version) walkOverlapping(aux tFiles, ikey internalKey, f func(level int, t *tFile) bool, lf func(level int) bool) {
	...
	if aux != nil {
		for _, t := range aux {
			if t.overlaps(v.s.ikc, ukey, ukey) {
				if !f(-1, t) {
					return
				}
			}
	...
	for level, tables := range v.levels {
		...
		if level == 0 {
			...
			// Find all files that overlap ukey.
			...
			if i := tables.searchMax(v.s.ikc, ikey); i < len(tables) {
				t := tables[i]
				if v.s.ikc.ucmp.Compare(ukey, t.imin.ukey()) >= 0 {
					if !f(level, t) {
						return
					}
				}
...
func (v *version) get(aux tFiles, ikey internalKey, ro *opt.ReadOptions, nov bool) (value []byte, tcomp bool, err error) {
	if v.closing {
	...
	// Entries never hop across level, so finding k/v in smaller
	...
	v.walkOverlapping(aux, ikey, func(level int, t *tFile) bool {
		...
		if fukey, fseq, fkt, fkerr := parseInternalKey(fikey); fkerr == nil {
			if v.s.ikc.ucmp.Compare(ukey, fukey) == 0 {
				...
				if level <= 0 {
					...
					case keyTypeDel:
					default:
						panic("granite: invalid internalKey type")
					}
					return false
				}
			}
...
func (v *version) getIterators(slice *util.Range, ro *opt.ReadOptions) (its []iterator.InternalIterator) {
	strict := opt.GetStrict(v.s.o.Options, ro, opt.StrictReader)
	for level, tables := range v.levels {
		if level == 0 {
			// merge all level-0 files together since they may overlap each other
			for _, t := range tables {
				its = append(its, v.s.tops.newIterator(t, slice, ro))
			}
		} else if len(tables) != 0 {
			its = append(its, iterator.NewIndexedIterator(
				tables.newIndexIterator(v.s.tops, v.s.ikc, slice, ro), strict),
			)
		}
	}
	return
...
func (v *version) offsetOf(ikey internalKey) (n int64, err error) {
	for level, tables := range v.levels {
		for _, t := range tables {
			if v.s.ikc.Compare(t.imax, ikey) <= 0 {
				// entire file is before 'ikey', so just add the file size
				n += t.size
			} else if v.s.ikc.Compare(t.imin, ikey) > 0 {
				// entire file is after 'ikey', so ignore
				if level > 0 {
					// files other than level-0 are sorted by meta->min, so
					// no further files in this level will contain data
					// for 'ikey'
					break
				}
			} else {
...
func (v *version) pickMemTableLevel(umin, umax []byte, maxLevel int) (level int) {
	if maxLevel > 0 {
		if len(v.levels) == 0 {
			return maxLevel
		}
		if !v.levels[0].overlaps(v.s.ikc, umin, umax, true) {
			var overlaps tFiles
			for ; level < maxLevel; level++ {
				if pLevel := level + 1; pLevel >= len(v.levels) {
					return maxLevel
				} else if v.levels[pLevel].overlaps(v.s.ikc, umin, umax, false) {
					break
				}
				if gpLevel := level + 2; gpLevel < len(v.levels) {
					overlaps = v.levels[gpLevel].getOverlaps(overlaps, v.s.ikc, umin, umax, false)
...
func (v *version) computeCompaction() {
	...
	for level, tables := range v.levels {
		...
		size := tables.size()
		if level == 0 {
			// Special handling is applied to level-0 compression,
			// limiting the number of files rather than the number
			// of bytes, for the following reasons:
			//
			// 1. When the write buffer is large, it's best to avoid
			// excessive level-0 compression.
			//
			// 2. Level-0 files are merged on each read, preventing
			// the merging of too many files when individual
			// files are small.
		...
		if level < len(p.levels) {
			...
			if trivial && len(scratch.added) > 0 {
				...
				if level == 0 {
					...
					added.sortByKey(p.base.s.ikc)
					_, amax := added.getRange(p.base.s.ikc)
					index := nt.searchMin(p.base.s.ikc, amax)
					nt = append(nt[:index], append(added, nt[index:]...)...)
				}
				nv.levels[level] = nt
				continue
